...
//#define LOG_NDEBUG 0 //Enable this to see verbose logs
...
status_t QCameraHardwareInterface::runFaceDetection()
{
    ...
    if (str != NULL) {
        ...
        ALOGV("%s: why set_dimension everytime?", __func__);
...
status_t QCameraHardwareInterface::setSceneMode(const QCameraParameters& params)
{
    ...
    if (str != NULL && oldstr != NULL) {
        int32_t value = attr_lookup(scenemode, sizeof(scenemode) / sizeof(str_map), str);
        ALOGD("Setting Scenemode value = %d",value );
        if (value != NOT_FOUND) {
            len = strlen(str);
            tmp = strlen(oldstr);
            len = (len < tmp) ? len : tmp;
            ...
            if(strncmp(str, oldstr, len)) {
                ...
                if(!strncmp(str, "hdr", 3) || !strncmp(oldstr, "hdr", 3) ) {
                    ALOGD("Changed between HDR/non-HDR states");
                    /* Restart only if preview already running*/
                    if (mPreviewState == QCAMERA_HAL_PREVIEW_STARTED) {
                        ALOGD("Preview in progress,restarting for HDR transition");
...
//        return NO_ERROR;
    ...
    if (str != NULL) {
        ...
        if ((value != NOT_FOUND) &&  (mDenoiseValue != value)) {
            ...
            ALOGD("Denoise enable=%d, plates=%d", temp.denoise_enable, temp.process_plates);
...
status_t QCameraHardwareInterface::setVideoSize(const QCameraParameters& params)
{
    ...
    if(!str) {
        ...
        ALOGD("%s: requested record size %s", __func__, str);
        if(!parse_size(str, videoWidth, videoHeight)) {
            ...
            if(old_vid_w != videoWidth || old_vid_h != videoHeight) {
                ...
                ALOGD("%s: Video sizes changes, Restart preview...", __func__);
            }
    ...
    ALOGD("%s: preview dimensions: %dx%d", __func__, mPreviewWidth, mPreviewHeight);
    ALOGD("%s: video dimensions: %dx%d", __func__, videoWidth, videoHeight);
    ...
    for (size_t i = 0; i <  mPreviewSizeCount; ++i) {
        if (width ==  mPreviewSizes[i].width
           && height ==  mPreviewSizes[i].height) {
            ...
            ALOGD("setPreviewSize:  width: %d   heigh: %d", width, height);
            mPreviewWidth = width;
...
status_t QCameraHardwareInterface::setPreviewFpsRange(const QCameraParameters& params)
{
    ...
    for(size_t i=0;i<FPS_RANGES_SUPPORTED_COUNT;i++)
    {
        if(minFps==FpsRangesSupported[i].minFPS && maxFps == FpsRangesSupported[i].maxFPS){
            ALOGD("FPS: i=%d : minFps = %d, maxFps = %d ",i,FpsRangesSupported[i].minFPS,FpsRangesSupported[i].maxFPS );
...
status_t QCameraHardwareInterface::setJpegThumbnailSize(const QCameraParameters& params){
    ...
    ALOGD("requested jpeg thumbnail size %d x %d", width, height);
...
status_t QCameraHardwareInterface::setPictureSize(const QCameraParameters& params)
{
    ...
    ALOGD("requested picture size %d x %d", width, height);
...
setNumOfSnapshot(const QCameraParameters& params) {
    ...
    ALOGD("number of snapshots = %d", num_of_snapshot);
...
status_t QCameraHardwareInterface::setPreviewFormat(const QCameraParameters& params) {
    ...
    if(previewFormat != NOT_FOUND) {
        ...
        ALOGD("Setting preview format to %d",mPreviewFormat);
        return NO_ERROR;
    ...
    if(str != NULL) {
        ...
        if(str_size == 2) {
            if(!strncmp(str, "on", str_size) || !strncmp(str, "ON", str_size)){
                ALOGD("Resetting mUseOverlay to false");
                strTexturesOn = true;
                mUseOverlay = false;
            }
        }else if(str_size == 3){
...
status_t QCameraHardwareInterface::setFaceDetect(const QCameraParameters& params)
{
    ...
    ALOGD("setFaceDetect: %s", str);
    if (str != NULL) {
        int value = attr_lookup(facedetection,
                sizeof(facedetection) / sizeof(str_map), str);
        mFaceDetectOn = value;
        ALOGD("%s Face detection value = %d",__func__, value);
        cam_ctrl_dimension_t dim;
//        cam_config_get_parm(mCameraId, MM_CAMERA_PARM_DIMENSION,&dim);
//        preview_parm_config (&dim, mParameters);
...
int QCameraHardwareInterface::getNumOfSnapshots(void) const
{
    ...
    if (atoi(prop)) {
        ALOGD("%s: Reading maximum no of snapshots = %d"
    ...
    if (histogram_en == QCAMERA_PARM_ENABLE) {
        ...
        for(int cnt = 0; cnt<3; cnt++) {
                ...
                if(mStatsMapped[cnt] == NULL) {
                   ...
                   ALOGD("Received following info for stats mapped data:%p,handle:%p, size:%d,release:%p",
...
void QCameraHardwareInterface::setExifTags()
{
    ...
    if(timeinfo != NULL) {
       ...
       ALOGE("EXIF DATETIME is not set because timeinfo is null");
    }
...
void QCameraHardwareInterface::setExifTagsGPS()
{
    ...
    if(str != NULL) {
       ...
       ALOGD("EXIFTAGID_GPS_PROCESSINGMETHOD = %s %s", mExifValues.gpsProcessingMethod,
    ...
    if(str != NULL) {
        ...
        ALOGD("EXIFTAGID_GPS_LATITUDE = %s", str);
        ...
        ALOGD("EXIFTAGID_GPS_LATITUDE_REF = %s", mExifValues.latRef);
    }else{
    ...
    if(str != NULL) {
        ...
        ALOGD("EXIFTAGID_GPS_LONGITUDE = %s", str);
        ...
        ALOGD("EXIFTAGID_GPS_LONGITUDE_REF = %s", mExifValues.lonRef);
    ...
    if(str != NULL) {
        ...
        ALOGD("EXIFTAGID_GPS_ALTITUDE = %f", value);
    }else{
