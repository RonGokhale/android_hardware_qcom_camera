...
/* get rdi size, by qury mm_camera*/
  ...
  ret = getBufferRdi( );
  if(ret != NO_ERROR) {
    LOGE("%s: cannot get memory from heap, ret = %d", __func__, ret);
    return ret;
  }
  ...
  for(int i = 0; i < mRdiStreamBuf.num; i++){
    ...
    mRdiStreamBuf.frame[i].ion_dev_fd = mHalCamCtrl->mRdiMemory.main_ion_fd[i];
    mRdiStreamBuf.frame[i].fd_data = mHalCamCtrl->mRdiMemory.ion_info_fd[i];
  ...
  mm_camera_ch_data_buf_t *frame)
{
  ...
  if(!mActive) {
    LOGE("RDI Streaming Stopped. Returning callback");
    return NO_ERROR;
  }
  if(mHalCamCtrl==NULL) {
...
status_t QCameraStream_Rdi::start()
{
    ...
    /* call mm_camera action start(...)  */
     ...
     LOGE("Starting RDI Stream. ");
     ret = cam_ops_action(mCameraId, TRUE, MM_CAMERA_OPS_RDI, 0);
     if (MM_CAMERA_OK != ret) {
       LOGE ("%s: rdi streaming start err=%d\n", __func__, ret);
        ret = BAD_VALUE;
        freeBufferRdi();
        goto end;
     }
...
// ---------------------------------------------------------------------------
  void QCameraStream_Rdi::release() {
    ...
    LOGE(": %s : De init Channel",__func__);
    if(ret != MM_CAMERA_OK) {
