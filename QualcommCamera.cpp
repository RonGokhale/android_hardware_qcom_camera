...
{
  ALOGI("Q%s: E window = %p", __func__, window);
  int rc = -1;
  QualcommCameraHardware *hardware = util_get_Hal_obj(device);
  if(hardware != NULL) {
        ...
        void *user)
{
  ...
  if(hardware != NULL){
    ...
    if(camHal) {
      ...
      #endif
      ALOGI("cam_nt_cb =%p,cam_dt_cb=%p,cam_dt_timestamp_cb=%p",  cam_nt_cb, cam_dt_cb, cam_dt_timestamp_cb);
